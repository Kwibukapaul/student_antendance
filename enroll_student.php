...
<head>
    ...
    <!-- Add your meta tags, links to external libraries, and other head content here -->
            ...
            <div class="container">
                ...
                <form method="POST">
                    <div class="form-floating mb-3">
                        <select class="form-select" id="floatingModules" name="course_id">
                            ...
                            try {
                                // Query to fetch modules
                                $stmt_modules = $pdo->query("SELECT id, course_name FROM modules");

                                // Fetching and displaying options
                                while ($module = $stmt_modules->fetch(PDO::FETCH_ASSOC)) {
                                    echo "<option value='{$module['id']}'>{$module['course_name']}</option>";
                                }
                            } catch (PDOException $e) {
                                // Displaying error message if query fails
                                echo "<option>Error fetching modules: {$e->getMessage()}</option>";
                            }
                            ?>
                        </select>
                        <label for="floatingModules">Course</label>
                    </div>
                    <button type="submit" class="btn btn-primary" name="save">Enroll</button>
                </form>
            </div>
        </div>
        <!-- Centered Form End -->
...
if(isset($_GET['id'])) {
    ...
    try {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" ) {
            ...
            // Prepare and execute the SQL query to insert into student_course table
